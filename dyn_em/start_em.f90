...
nba_mij,nba_rij,sbmradar,chem &
   ...
   USE module_implicit_sdirk3, ONLY : sdirk3_reset_on_restart
...
real      ,DIMENSION(grid%sm31:grid%em31,grid%sm32:grid%em32,grid%sm33:grid%em33,num_chem)           :: chem
   ...
   IF (config_flags%time_integration_scheme == 6) THEN
      CALL wrf_message('Initializing SDIRK3 implicit time integration in start_em')
