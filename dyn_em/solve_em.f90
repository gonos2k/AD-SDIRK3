...
real      ,DIMENSION(grid%sm31:grid%em31,grid%sm32:grid%em32,grid%sm33:grid%em33,num_chem)           :: chem_old
   ...
   INTEGER :: sdirk3_final_step
   INTEGER :: sdirk3_tile, sdirk3_checkpoint_count, sdirk3_global_timestep
   INTEGER :: sdirk3_state_size, sdirk3_ierr, sdirk3_checkpoints_used
   REAL, PARAMETER :: sdirk3_adjoint_gamma = 0.4358665
   REAL, ALLOCATABLE, DIMENSION(:) :: sdirk3_lambda_terminal, sdirk3_lambda_initial
   ...
   WRITE(0,*) '=== FORTRAN DEBUG: About to check USE_SDIRK3 block ==='
   WRITE(0,*) '=== FORTRAN DEBUG: config_flags%time_integration_scheme =', config_flags%time_integration_scheme
...
CALL PERIOD_BDY_EM_MOIST_OLD_sub ( grid, &
  ...
  ips, ipe, jps, jpe, kps, kpe )
   ...
   IF (config_flags%time_integration_scheme == 6) THEN
      WRITE(0,*) '=== SDIRK3 FORTRAN: Entering main SDIRK3 computation ==='
      WRITE(0,*) '=== SDIRK3 FORTRAN: itimestep =', grid%itimestep
      WRITE(0,*) '=== SDIRK3 FORTRAN: num_tiles =', grid%num_tiles
      WRITE(0,*) '=== SDIRK3 FORTRAN: checking initialization...'
      CALL FLUSH(0)
      
      
      IF (grid%itimestep == 1 .AND. grid%num_tiles > 0) THEN
         CALL wrf_message('Using SDIRK3 implicit time integration')
         
         
         
         
         WRITE(0,*) '=== BEFORE sdirk3_initialize_solvers ==='
         CALL FLUSH(0)
         CALL sdirk3_initialize_solvers(grid, config_flags)
         WRITE(0,*) '=== AFTER sdirk3_initialize_solvers ==='
         CALL FLUSH(0)
         
         
         WRITE(0,*) '=== BEFORE set_base_state_sdirk3 ==='
         CALL FLUSH(0)
         CALL set_base_state_sdirk3(grid, config_flags, &
                                   ids, ide, jds, jde, kds, kde, &
                                   ims, ime, jms, jme, kms, kme)
         WRITE(0,*) '=== AFTER set_base_state_sdirk3 ==='
         CALL FLUSH(0)
      END IF
      
      
      CALL sdirk3_prepare_physics_coupling(grid, config_flags)
      ...
      IF (config_flags%mp_physics == 0 .OR. num_3d_m <= 0) THEN
         DO j = jps, jpe
            DO k = k_start, k_end
               DO i = ips, ipe
                  cqu(i,k,j) = 1.0
      ...
      ELSE
         ...
         CALL calc_cq ( moist, cqu, cqv, cqw, num_3d_m, &
                        ids, ide, jds, jde, kds, kde,     &
                        ims, ime, jms, jme, kms, kme,     &
                        ips, ipe, jps, jpe, k_start, k_end )

         
         
         IF (MAXVAL(ABS(cqu(ips:ipe,k_start:k_end,jps:jpe))) > 1.0E10 .OR. &
             MAXVAL(ABS(cqw(ips:ipe,k_start:k_end,jps:jpe))) > 1.0E10) THEN
            WRITE(0,*) 'WARNING: Invalid cqu/cqw values detected in SDIRK3 path'
            WRITE(0,*) 'Setting to dry dynamics values (cqu=1.0, cqv=1.0, cqw=0.0)'
            WRITE(0,*) 'This typically indicates a dry dynamics test case'
            DO j = jps, jpe
               DO k = k_start, k_end
                  DO i = ips, ipe
                     cqu(i,k,j) = 1.0
      ...
      IF (config_flags%sdirk3_debug_level > 0) THEN
         WRITE(0,*) '=== DEBUG: After calc_cq in solve_em ==='
         WRITE(0,*) 'num_3d_m = ', num_3d_m
         WRITE(0,*) 'cqu(ips,k_start,jps) = ', cqu(ips,k_start,jps)
         WRITE(0,*) 'cqv(ips,k_start,jps) = ', cqv(ips,k_start,jps)
         WRITE(0,*) 'cqw(ips,k_start,jps) = ', cqw(ips,k_start,jps)
         WRITE(0,*) 'cqu min/max: ', MINVAL(cqu(ips:ipe,k_start:k_end,jps:jpe)), &
                                     MAXVAL(cqu(ips:ipe,k_start:k_end,jps:jpe))
      ENDIF
      ...
      DO rk_step = 1, 3
         CALL advance_implicit(grid, config_flags, rk_step, grid%dt, &
                               cqu, cqv, cqw,                         &
                               ids, ide, jds, jde, kds, kde,         &
                               ims, ime, jms, jme, kms, kme)
         ...
         IF (rk_step == 1) THEN
            IF (config_flags%h_mom_adv_order <= 6 .and. config_flags%h_sca_adv_order <= 6) THEN
...
CALL HALO_EM_D2_5_sub ( grid, &
  ...
  ips, ipe, jps, jpe, kps, kpe )
            ...
            ELSE IF (config_flags%h_mom_adv_order <= 4 .and. config_flags%h_sca_adv_order <= 4) THEN
...
CALL HALO_EM_D2_3_sub ( grid, &
  ...
  ips, ipe, jps, jpe, kps, kpe )
      ...
      CALL sdirk3_apply_physics_tendencies(grid, config_flags, grid%dt)
...
CALL PERIOD_BDY_EM_D_sub ( grid, &
  ...
  ips, ipe, jps, jpe, kps, kpe )
      ...
      !$OMP PRIVATE ( ij )
      DO ij = 1 , grid%num_tiles
         CALL wrf_debug ( 200 , ' SDIRK3: calling rk_phys_bc_dry_2' )
      ...
      IF (MOD(grid%itimestep, 100) == 0) THEN
         CALL sdirk3_print_solver_statistics()
      END IF
      
      
      CALL wrf_debug(1, 'SDIRK3 timestep complete with BCs applied, skipping RK loop')
      ...
      grid%mut(ips:ipe, jps:jpe) = grid%mub(ips:ipe, jps:jpe) + grid%mu_2(ips:ipe, jps:jpe)
      grid%muts(ips:ipe, jps:jpe) = grid%mub(ips:ipe, jps:jpe) + grid%mu_2(ips:ipe, jps:jpe)
  ...
  ips, ipe, jps, jpe, kps, kpe )
     ...
     CALL microphysics_driver(                                            &
      ...
      &        , QAOLI3_CURR=scalar(ims,kms,jms,P_QAOLI3), F_QAOLI3=F_QAOLI3  & 
       &        , QZR_CURR=scalar(ims,kms,jms,P_QZR), F_QZR=F_QZR          & 
...
CALL HALO_FIREBRAND_SPOTTING_5_sub ( grid, &
  ...
  ips, ipe, jps, jpe, kps, kpe )
   ...
   IF (config_flags%time_integration_scheme == 6) THEN
      sdirk3_final_step = grid%run_days*86400/grid%dt + &
                          grid%run_hours*3600/grid%dt + &
                          grid%run_minutes*60/grid%dt + &
                          grid%run_seconds/grid%dt
      IF (grid%itimestep == sdirk3_final_step) THEN

         
         
         IF (config_flags%sdirk3_save_trajectory) THEN
            CALL wrf_message(' ')
            CALL wrf_message('SDIRK-3: Running no-observation 4DVAR replay loop for saved window.')
            DO sdirk3_tile = 1, grid%num_tiles
               CALL sdirk3_get_4dvar_trajectory_info(sdirk3_tile, sdirk3_checkpoint_count, &
                                                     sdirk3_global_timestep, sdirk3_state_size, sdirk3_ierr)

               IF (sdirk3_ierr /= 0 .OR. sdirk3_state_size <= 0) THEN
                  WRITE(wrf_err_message,'(A,I4,A)') 'SDIRK-3 4DVAR: tile ', sdirk3_tile, &
                       ' trajectory info unavailable, skipping replay.'
                  CALL wrf_message(TRIM(wrf_err_message))
               ELSE IF (sdirk3_checkpoint_count <= 0) THEN
                  WRITE(wrf_err_message,'(A,I4,A)') 'SDIRK-3 4DVAR: tile ', sdirk3_tile, &
                       ' has no saved checkpoints, skipping replay.'
                  CALL wrf_message(TRIM(wrf_err_message))
               ELSE
                  ALLOCATE(sdirk3_lambda_terminal(sdirk3_state_size))
                  ALLOCATE(sdirk3_lambda_initial(sdirk3_state_size))
                  sdirk3_lambda_terminal = 0.0
                  sdirk3_lambda_initial = 0.0

                  CALL sdirk3_run_4dvar_adjoint_loop(sdirk3_tile, &
                                                     sdirk3_lambda_terminal, sdirk3_lambda_initial, &
                                                     sdirk3_state_size, grid%dt, sdirk3_adjoint_gamma, &
                                                     15, 80, 1.0E-5, sdirk3_checkpoints_used, sdirk3_ierr)

                  IF (sdirk3_ierr == 0) THEN
                     WRITE(wrf_err_message,'(A,I4,A,I8,A,I8,A)') 'SDIRK-3 4DVAR: tile ', sdirk3_tile, &
                          ' replay complete. checkpoints_used=', sdirk3_checkpoints_used, &
                          ', global_timestep=', sdirk3_global_timestep, '.'
                  ELSE
                     WRITE(wrf_err_message,'(A,I4,A)') 'SDIRK-3 4DVAR: tile ', sdirk3_tile, &
                          ' replay failed.'
                  END IF
                  ...
                  DEALLOCATE(sdirk3_lambda_terminal)
                  DEALLOCATE(sdirk3_lambda_initial)
               END IF
            END DO
         END IF
         ...
         CALL wrf_message(' ')
         CALL wrf_message('SDIRK-3: Final solver statistics at end of run:')
         CALL sdirk3_print_solver_statistics()
         
         
         CALL finalize_implicit_sdirk3()
         
         WRITE(wrf_err_message,'(A)') 'SDIRK-3: All solvers finalized at end of run'
