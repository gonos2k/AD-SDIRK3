...
&cs,SF_SURFACE_PHYSICS=config_flags%sf_surface_physics,SH2O=grid%sh2o,SHDMAX=grid%shdmax,SHDMIN=grid%shdmin,SMOIS=grid%smois,SHDAVG&
&=grid%shdavg,SMSTAV=grid%smstav,SMSTOT=grid%smstot,SNOALB=grid%snoalb,SNOW=grid%snow,SNOWC=grid%snowc,SNOWH=grid%snowh,SMCREL=grid&
&%smcrel,SST=grid%sst,SST_INPUT=grid%sst_input,SST_UPDATE=grid%sst_update,SSTSK=grid%sstsk,DTW=grid%dtw,SST_SKIN=grid%sst_skin,SCM_&
&FORCE_SKINTEMP=grid%scm_force_skintemp,SCM_FORCE_FLUX=grid%scm_force_flux,STEPBL=grid%stepbl,TH10=grid%th10,TH2=grid%th2,THZ0=grid&
&%thz0,TH_PHY=th_phy,TKE_PBL=grid%tke_pbl,TMN=grid%tmn,TSHLTR=grid%tshltr,TSK=grid%tsk,TYR=grid%tyr,TYRA=grid%tyra,TDLY=grid%tdly,T&
&LAG=grid%tlag,LAGDAY=config_flags%lagday,NYEAR=grid%nyear,NDAY=grid%nday,TMN_UPDATE=grid%tmn_update,YR=yr,TSLB=grid%tslb,T_PHY=t_p&
&hy,U10=grid%u10,URATX=grid%uratx,VRATX=grid%vratx,TRATX=grid%tratx,UDRUNOFF=grid%udrunoff,UST=grid%ust,UZ0=grid%uz0,U_FRAME=grid%u&
&_frame,U_PHY=grid%u_phy,V10=grid%v10,U10E=grid%u10e,V10E=grid%v10e,UOCE=grid%uoce,VOCE=grid%voce,VEGFRA=grid%vegfra,VZ0=grid%vz0,V&
&_FRAME=grid%v_frame,V_PHY=grid%v_phy,WARM_RAIN=grid%warm_rain,WSPD=grid%wspd,XICE=grid%xice,XLAND=grid%xland,MAX_EDOM=grid%num_ext&
&_model_couple_dom,CPLMASK=grid%cplmask,Z0=grid%z0,Z=grid%z,ZNT=grid%znt,ZS=grid%zs,ALBSI=grid%albsi,ICEDEPTH=grid%icedepth,SNOWSI=&
&grid%snowsi,XICEM=grid%xicem,ISICE=grid%landuse_isice,USTM=grid%ustm,CK=grid%ck,CKA=grid%cka,CD=grid%cd,CDA=grid%cda,ISFTCFLX=conf&
&ig_flags%isftcflx,IZ0TLND=config_flags%iz0tlnd,SF_OCEAN_PHYSICS=config_flags%sf_ocean_physics,OML_HML0=config_flags%oml_hml0,OML_G&
&AMMA=config_flags%oml_gamma,TML=grid%tml,T0ML=grid%t0ml,HML=grid%hml,H0ML=grid%h0ml,HUML=grid%huml,HVML=grid%hvml,F=grid%f,TMOML=g&
&rid%TMOML,ISWATER=iswater,OML_RELAXATION_TIME=grid%OML_RELAXATION_TIME,shalwater_z0=config_flags%shalwater_z0,water_depth=grid%wat&
&er_depth,shalwater_depth=config_flags%shalwater_depth,lakedepth2d=grid%lakedepth2d,savedtke12d=grid%savedtke12d,snowdp2d=grid%snow&
&dp2d,h2osno2d=grid%h2osno2d,snl2d=grid%snl2d,t_grnd2d=grid%t_grnd2d,t_lake3d=grid%t_lake3d,lake_icefrac3d=grid%lake_icefrac3d,z_la&
&ke3d=grid%z_lake3d,dz_lake3d=grid%dz_lake3d,t_soisno3d=grid%t_soisno3d,h2osoi_ice3d=grid%h2osoi_ice3d,h2osoi_liq3d=grid%h2osoi_liq&
&3d,h2osoi_vol3d=grid%h2osoi_vol3d,z3d=grid%z3d,dz3d=grid%dz3d,zi3d=grid%zi3d,watsat3d=grid%watsat3d,csol3d=grid%csol3d,tkmg3d=grid&
&%tkmg3d,tkdry3d=grid%tkdry3d,tksatu3d=grid%tksatu3d,LakeModel=grid%sf_lake_physics,lake_min_elev=grid%lake_min_elev,LakeMask=grid%&
&LakeMask,restart_flag=restart_flag,NUMC=grid%numc,NUMP=grid%nump,SABV=grid%sabv,SABG=grid%sabg,LWUP=grid%lwup,SNL=grid%snl,HISTORY&
&_INTERVAL=config_flags%history_interval,SNOWDP=grid%snowdp,WTC=grid%wtc,WTP=grid%wtp,H2OSNO=grid%h2osno,T_GRND=grid%t_grnd,T_VEG=g&
&rid%t_veg,H2OCAN=grid%h2ocan,H2OCAN_COL=grid%h2ocan_col,T2M_MAX=grid%t2m_max,T2M_MIN=grid%t2m_min,T2CLM=grid%t2clm,T_REF2M=grid%t_&
&ref2m,H2OSOI_LIQ_S1=grid%h2osoi_liq_s1,H2OSOI_LIQ_S2=grid%h2osoi_liq_s2,H2OSOI_LIQ_S3=grid%h2osoi_liq_s3,H2OSOI_LIQ_S4=grid%h2osoi&
&_liq_s4,H2OSOI_LIQ_S5=grid%h2osoi_liq_s5,H2OSOI_LIQ1=grid%h2osoi_liq1,H2OSOI_LIQ2=grid%h2osoi_liq2,H2OSOI_LIQ3=grid%h2osoi_liq3,H2&
&OSOI_LIQ4=grid%h2osoi_liq4,H2OSOI_LIQ5=grid%h2osoi_liq5,H2OSOI_LIQ6=grid%h2osoi_liq6,H2OSOI_LIQ7=grid%h2osoi_liq7,H2OSOI_LIQ8=grid&
&%h2osoi_liq8,H2OSOI_LIQ9=grid%h2osoi_liq9,H2OSOI_LIQ10=grid%h2osoi_liq10,H2OSOI_ICE_S1=grid%h2osoi_ice_s1,H2OSOI_ICE_S2=grid%h2oso&
&i_ice_s2,H2OSOI_ICE_S3=grid%h2osoi_ice_s3,H2OSOI_ICE_S4=grid%h2osoi_ice_s4,H2OSOI_ICE_S5=grid%h2osoi_ice_s5,H2OSOI_ICE1=grid%h2oso&
&i_ice1,H2OSOI_ICE2=grid%h2osoi_ice2,H2OSOI_ICE3=grid%h2osoi_ice3,H2OSOI_ICE4=grid%h2osoi_ice4,H2OSOI_ICE5=grid%h2osoi_ice5,H2OSOI_&
&ICE6=grid%h2osoi_ice6,H2OSOI_ICE7=grid%h2osoi_ice7,H2OSOI_ICE8=grid%h2osoi_ice8,H2OSOI_ICE9=grid%h2osoi_ice9,H2OSOI_ICE10=grid%h2o&
&soi_ice10,T_SOISNO_S1=grid%t_soisno_s1,T_SOISNO_S2=grid%t_soisno_s2,T_SOISNO_S3=grid%t_soisno_s3,T_SOISNO_S4=grid%t_soisno_s4,T_SO&
&ISNO_S5=grid%t_soisno_s5,T_SOISNO1=grid%t_soisno1,T_SOISNO2=grid%t_soisno2,T_SOISNO3=grid%t_soisno3,T_SOISNO4=grid%t_soisno4,T_SOI&
&SNO5=grid%t_soisno5,T_SOISNO6=grid%t_soisno6,T_SOISNO7=grid%t_soisno7,T_SOISNO8=grid%t_soisno8,T_SOISNO9=grid%t_soisno9,T_SOISNO10&
&=grid%t_soisno10,DZSNOW1=grid%dzsnow1,DZSNOW2=grid%dzsnow2,DZSNOW3=grid%dzsnow3,DZSNOW4=grid%dzsnow4,DZSNOW5=grid%dzsnow5,SNOWRDS1&
&=grid%snowrds1,SNOWRDS2=grid%snowrds2,SNOWRDS3=grid%snowrds3,SNOWRDS4=grid%snowrds4,SNOWRDS5=grid%snowrds5,T_LAKE1=grid%t_lake1,T_&
&LAKE2=grid%t_lake2,T_LAKE3=grid%t_lake3,T_LAKE4=grid%t_lake4,T_LAKE5=grid%t_lake5,T_LAKE6=grid%t_lake6,T_LAKE7=grid%t_lake7,T_LAKE&
&8=grid%t_lake8,T_LAKE9=grid%t_lake9,T_LAKE10=grid%t_lake10,H2OSOI_VOL1=grid%h2osoi_vol1,H2OSOI_VOL2=grid%h2osoi_vol2,H2OSOI_VOL3=g&
&rid%h2osoi_vol3,H2OSOI_VOL4=grid%h2osoi_vol4,H2OSOI_VOL5=grid%h2osoi_vol5,H2OSOI_VOL6=grid%h2osoi_vol6,H2OSOI_VOL7=grid%h2osoi_vol&
&7,H2OSOI_VOL8=grid%h2osoi_vol8,H2OSOI_VOL9=grid%h2osoi_vol9,H2OSOI_VOL10=grid%h2osoi_vol10,MAXPATCH=config_flags%maxpatch,INEST=gr&
&id%id,ALBEDOsubgrid=grid%ALBEDOsubgrid,LHsubgrid=grid%LHsubgrid,HFXsubgrid=grid%HFXsubgrid,LWUPsubgrid=grid%LWUPsubgrid,Q2subgrid=&
&grid%Q2subgrid,SABVsubgrid=grid%SABVsubgrid,SABGsubgrid=grid%SABGsubgrid,NRAsubgrid=grid%NRAsubgrid,SWUPsubgrid=grid%SWUPsubgrid,L&
&Hsoi=grid%LHsoi,LHveg=grid%LHveg,LHtran=grid%LHtran,t_veg24=grid%t_veg24,t_veg240=grid%t_veg240,fsun24=grid%fsun24,fsun240=grid%fs&
&un240,fsd24=grid%fsd24,fsd240=grid%fsd240,fsi24=grid%fsi24,fsi240=grid%fsi240,laip=grid%laip,pct_pft_input=grid%pct_pft_input,num_&
&pft_input=grid%num_pft_clm,input_pft_flag=config_flags%input_pft,SLOPE_RAD=config_flags%slope_rad,TOPO_SHADING=config_flags%topo_s&
&hading,SHADOWMASK=grid%shadowmask,DIFFUSE_FRAC=grid%diffuse_frac,SLOPE=grid%slope,SLP_AZI=grid%slp_azi,SWNORM=grid%swnorm,DECLIN=g&
&rid%declin,SOLCON=grid%solcon,COSZEN=grid%coszen,HRANG=grid%hrang,xlat_urb2d=grid%XLAT,NUM_ROOF_LAYERS=num_roof_layers,NUM_WALL_LA&
&YERS=num_wall_layers,NUM_ROAD_LAYERS=num_road_layers,DZR=grid%dzr,DZB=grid%dzb,DZG=grid%dzg,TR_URB2D=grid%tr_urb2d,TB_URB2D=grid%t&
&b_urb2d,TG_URB2D=grid%tg_urb2d,TC_URB2D=grid%tc_urb2d,QC_URB2D=grid%qc_urb2d,UC_URB2D=grid%uc_urb2d,XXXR_URB2D=grid%xxxr_urb2d,XXX&
&B_URB2D=grid%xxxb_urb2d,XXXG_URB2D=grid%xxxg_urb2d,XXXC_URB2D=grid%xxxc_urb2d,CMCR_URB2D=grid%cmcr_urb2d,TGR_URB2D=grid%tgr_urb2d,&
&TGRL_URB3D=grid%tgrl_urb3d,SMR_URB3D=grid%smr_urb3d,JULIAN=grid%julday,JULYR=grid%julyr,DRELR_URB2D=grid%drelr_urb2d,DRELB_URB2D=g&
&rid%drelb_urb2d,DRELG_URB2D=grid%drelg_urb2d,FLXHUMR_URB2D=grid%flxhumr_urb2d,FLXHUMB_URB2D=grid%flxhumb_urb2d,FLXHUMG_URB2D=grid%&
&flxhumg_urb2d,TRL_URB3D=grid%trl_urb3d,TBL_URB3D=grid%tbl_urb3d,TGL_URB3D=grid%tgl_urb3d,SH_URB2D=grid%sh_urb2d,LH_URB2D=grid%lh_u&
&rb2d,G_URB2D=grid%g_urb2d,RN_URB2D=grid%rn_urb2d,TS_URB2D=grid%ts_urb2d,FRC_URB2D=grid%frc_urb2d,UTYPE_URB2D=grid%utype_urb2d,SWDD&
&IR=grid%swddir,SWDDIF=grid%swddif,SF_URBAN_PHYSICS=config_flags%sf_urban_physics,num_urban_ndm=config_flags%num_urban_ndm,urban_ma&
&p_zrd=config_flags%urban_map_zrd,urban_map_zwd=config_flags%urban_map_zwd,urban_map_gd=config_flags%urban_map_gd,urban_map_zd=conf&
&ig_flags%urban_map_zd,urban_map_zdf=config_flags%urban_map_zdf,urban_map_bd=config_flags%urban_map_bd,urban_map_wd=config_flags%ur&
&ban_map_wd,urban_map_gbd=config_flags%urban_map_gbd,urban_map_fbd=config_flags%urban_map_fbd,urban_map_zgrd=config_flags%urban_map&
&_zgrd,NUM_URBAN_HI=config_flags%num_urban_hi,use_wudapt_lcz=config_flags%use_wudapt_lcz,slucm_distributed_drag=config_flags%slucm_&
&distributed_drag,TSK_RURAL=grid%tsk_rural,TRB_URB4D=grid%trb_urb4d,TW1_URB4D=grid%tw1_urb4d,TW2_URB4D=grid%tw2_urb4d,TGB_URB4D=gri&
&d%tgb_urb4d,TLEV_URB3D=grid%tlev_urb3d,QLEV_URB3D=grid%qlev_urb3d,TW1LEV_URB3D=grid%tw1lev_urb3d,TW2LEV_URB3D=grid%tw2lev_urb3d,TG&
&LEV_URB3D=grid%tglev_urb3d,TFLEV_URB3D=grid%tflev_urb3d,SF_AC_URB3D=grid%sf_ac_urb3d,LF_AC_URB3D=grid%lf_ac_urb3d,CM_AC_URB3D=grid&
&%cm_ac_urb3d,SFVENT_URB3D=grid%sfvent_urb3d,LFVENT_URB3D=grid%lfvent_urb3d,SFWIN1_URB3D=grid%sfwin1_urb3d,SFWIN2_URB3D=grid%sfwin2&
&_urb3d,SFW1_URB3D=grid%sfw1_urb3d,SFW2_URB3D=grid%sfw2_urb3d,SFR_URB3D=grid%sfr_urb3d,SFG_URB3D=grid%sfg_urb3d,EP_PV_URB3D=grid%ep&
&_pv_urb3d,T_PV_URB3D=grid%t_pv_urb3d,TRV_URB4D=grid%trv_urb4d,QR_URB4D=grid%qr_urb4d,QGR_URB3D=grid%qgr_urb3d,TGR_URB3D=grid%tgr_u&
&rb3d,DRAIN_URB4D=grid%drain_urb4d,DRAINGR_URB3D=grid%draingr_urb3d,SFRV_URB3D=grid%sfrv_urb3d,LFRV_URB3D=grid%lfrv_urb3d,DGR_URB3D&
&=grid%dgr_urb3d,DG_URB3D=grid%dg_urb3d,LFR_URB3D=grid%lfr_urb3d,LFG_URB3D=grid%lfg_urb3d,LP_URB2D=grid%lp_urb2d,HI_URB2D=grid%hi_u&
&rb2d,LB_URB2D=grid%lb_urb2d,HGT_URB2D=grid%hgt_urb2d,MH_URB2D=grid%mh_urb2d,STDH_URB2D=grid%stdh_urb2d,LF_URB2D=grid%lf_urb2d,lf_u&
&rb2d_s=grid%lf_urb2d_s,z0_urb2d=grid%z0_urb2d,GMT=grid%gmt,XLAT=grid%xlat,XLONG=grid%xlong,JULDAY=grid%julday,distributed_ahe_opt=&
&grid%distributed_ahe_opt,ahe=grid%ahe,A_U_BEP=grid%a_u_bep,A_V_BEP=grid%a_v_bep,A_T_BEP=grid%a_t_bep,A_Q_BEP=grid%a_q_bep,B_U_BEP=&
&grid%b_u_bep,B_V_BEP=grid%b_v_bep,B_T_BEP=grid%b_t_bep,B_Q_BEP=grid%b_q_bep,SF_BEP=grid%sf_bep,VL_BEP=grid%vl_bep,A_E_BEP=grid%a_e&
&_bep,B_E_BEP=grid%b_e_bep,DLG_BEP=grid%dlg_bep,DL_U_BEP=grid%dl_u_bep,CMR_SFCDIF=grid%cmr_sfcdif,CHR_SFCDIF=grid%chr_sfcdif,CMC_SF&
&CDIF=grid%cmc_sfcdif,CHC_SFCDIF=grid%chc_sfcdif,CMGR_SFCDIF=grid%cmgr_sfcdif,CHGR_SFCDIF=grid%chgr_sfcdif,LANDUSEF=grid%landusef,S&
&OILCTOP=grid%soilctop,SOILCBOT=grid%soilcbot,RA=grid%ra,RS=grid%rs,LAI=grid%lai,IMPERV=grid%imperv,CANFRA=grid%canfra,NLCAT=grid%n&
&um_land_cat,NSCAT=grid%num_soil_cat,VEGF_PX=grid%vegf_px,SNOWNCV=grid%snowncv,ANAL_INTERVAL=config_flags%auxinput9_interval_s+conf&
&ig_flags%auxinput9_interval_m*60,PXLSM_SMOIS_INIT=config_flags%pxlsm_smois_init,PXLSM_SOIL_NUDGE=config_flags%pxlsm_soil_nudge,als&
&wvisdir=grid%alswvisdir,alswvisdif=grid%alswvisdif,alswnirdir=grid%alswnirdir,alswnirdif=grid%alswnirdif,swvisdir=grid%swvisdir,sw&
&visdif=grid%swvisdif,swnirdir=grid%swnirdir,swnirdif=grid%swnirdif,ssib_br=grid%ssib_br,ssib_fm=grid%ssib_fm,ssib_fh=grid%ssib_fh,&
&ssib_cm=grid%ssib_cm,ssibxdd=grid%ssibxdd,ssib_lhf=grid%ssib_lhf,ssib_shf=grid%ssib_shf,ssib_ghf=grid%ssib_ghf,ssib_egs=grid%ssib_&
&egs,ssib_eci=grid%ssib_eci,ssib_ect=grid%ssib_ect,ssib_egi=grid%ssib_egi,ssib_egt=grid%ssib_egt,ssib_sdn=grid%ssib_sdn,ssib_sup=gr&
&id%ssib_sup,ssib_ldn=grid%ssib_ldn,ssib_lup=grid%ssib_lup,ssib_wat=grid%ssib_wat,ssib_shc=grid%ssib_shc,ssib_shg=grid%ssib_shg,ssi&
&b_lai=grid%ssib_lai,ssib_vcf=grid%ssib_vcf,ssib_z00=grid%ssib_z00,ssib_veg=grid%ssib_veg,cldfra=grid%cldfra,ISNOW=grid%isnow,SWE=g&
&rid%swe,SNOWDEN=grid%snowden,SNOWDEPTH=grid%snowdepth,TKAIR=grid%tkair,DZO1=grid%dzo1,WO1=grid%wo1,TSSN1=grid%tssn1,TSSNO1=grid%ts&
&sno1,BWO1=grid%bwo1,BTO1=grid%bto1,CTO1=grid%cto1,FIO1=grid%fio1,FLO1=grid%flo1,BIO1=grid%bio1,BLO1=grid%blo1,HO1=grid%ho1,DZO2=gr&
&id%dzo2,WO2=grid%wo2,TSSN2=grid%tssn2,TSSNO2=grid%tssno2,BWO2=grid%bwo2,BTO2=grid%bto2,CTO2=grid%cto2,FIO2=grid%fio2,FLO2=grid%flo&
&2,BIO2=grid%bio2,BLO2=grid%blo2,HO2=grid%ho2,DZO3=grid%dzo3,WO3=grid%wo3,TSSN3=grid%tssn3,TSSNO3=grid%tssno3,BWO3=grid%bwo3,BTO3=g&
&rid%bto3,CTO3=grid%cto3,FIO3=grid%fio3,FLO3=grid%flo3,BIO3=grid%bio3,BLO3=grid%blo3,HO3=grid%ho3,DZO4=grid%dzo4,WO4=grid%wo4,TSSN4&
...
&%blo4,HO4=grid%ho4,RA_SW_PHYSICS=config_flags%ra_sw_physics,t2_ndg_old=grid%t2_ndg_old,q2_ndg_old=grid%q2_ndg_old,t2_ndg_new=grid%&
&t2_ndg_new,q2_ndg_new=grid%q2_ndg_new,sn_ndg_old=grid%sn_ndg_old,sn_ndg_new=grid%sn_ndg_new,pxlsm_modis_veg=config_flags%pxlsm_mod&
&is_veg,LAI_PX=grid%lai_px,WWLT_PX=grid%wwlt_px,WFC_PX=grid%wfc_px,WSAT_PX=grid%wsat_px,CLAY_PX=grid%clay_px,CSAND_PX=grid%csand_px&
&,FMSAND_PX=grid%fmsand_px,idveg=config_flags%dveg,iopt_crs=config_flags%opt_crs,iopt_btr=config_flags%opt_btr,iopt_run=config_flag&
&s%opt_run,iopt_sfc=config_flags%opt_sfc,iopt_frz=config_flags%opt_frz,iopt_inf=config_flags%opt_inf,iopt_rad=config_flags%opt_rad,&
&iopt_alb=config_flags%opt_alb,iopt_snf=config_flags%opt_snf,iopt_tbot=config_flags%opt_tbot,iopt_stc=config_flags%opt_stc,iopt_gla&
&=config_flags%opt_gla,iopt_rsf=config_flags%opt_rsf,iopt_soil=config_flags%opt_soil,iopt_pedo=config_flags%opt_pedo,iopt_crop=conf&
&ig_flags%opt_crop,iopt_irr=config_flags%opt_irr,iopt_irrm=config_flags%opt_irrm,iopt_infdv=config_flags%opt_infdv,iopt_tdrn=config&
&_flags%opt_tdrn,soiltstep=config_flags%soiltstep,isnowxy=grid%isnowxy,tvxy=grid%tvxy,tgxy=grid%tgxy,canicexy=grid%canicexy,canliqx&
&y=grid%canliqxy,eahxy=grid%eahxy,tahxy=grid%tahxy,cmxy=grid%cmxy,chxy=grid%chxy,fwetxy=grid%fwetxy,sneqvoxy=grid%sneqvoxy,alboldxy&
&=grid%alboldxy,qsnowxy=grid%qsnowxy,qrainxy=grid%qrainxy,wslakexy=grid%wslakexy,zwtxy=grid%zwtxy,waxy=grid%waxy,wtxy=grid%wtxy,tsn&
&oxy=grid%tsnoxy,zsnsoxy=grid%zsnsoxy,snicexy=grid%snicexy,snliqxy=grid%snliqxy,lfmassxy=grid%lfmassxy,rtmassxy=grid%rtmassxy,stmas&
&sxy=grid%stmassxy,woodxy=grid%woodxy,stblcpxy=grid%stblcpxy,fastcpxy=grid%fastcpxy,grainxy=grid%grainxy,gddxy=grid%gddxy,pgsxy=gri&
&d%pgsxy,cropcat=grid%cropcat,planting=grid%planting,harvest=grid%harvest,season_gdd=grid%season_gdd,soilcomp=grid%soilcomp,soilcl1&
&=grid%soilcl1,soilcl2=grid%soilcl2,soilcl3=grid%soilcl3,soilcl4=grid%soilcl4,xsaixy=grid%xsaixy,taussxy=grid%taussxy,t2mvxy=grid%t&
...
&d%chb2xy,chstarxy=grid%chstarxy,qintsxy=grid%qintsxy,qintrxy=grid%qintrxy,qdripsxy=grid%qdripsxy,qdriprxy=grid%qdriprxy,qthrosxy=g&
&rid%qthrosxy,qthrorxy=grid%qthrorxy,qsnsubxy=grid%qsnsubxy,qsnfroxy=grid%qsnfroxy,qsubcxy=grid%qsubcxy,qfrocxy=grid%qfrocxy,qfrzcx&
&y=grid%qfrzcxy,qmeltcxy=grid%qmeltcxy,qsnbotxy=grid%qsnbotxy,pondingxy=grid%pondingxy,PAHXY=grid%PAHXY,PAHGXY=grid%PAHGXY,PAHVXY=g&
&rid%PAHVXY,PAHBXY=grid%PAHBXY,qmeltxy=grid%qmeltxy,FPICEXY=grid%FPICEXY,qevacxy=grid%qevacxy,qdewcxy=grid%qdewcxy,RAINLSM=grid%RAI&
&NLSM,SNOWLSM=grid%SNOWLSM,forctlsm=grid%forctlsm,forcqlsm=grid%forcqlsm,forcplsm=grid%forcplsm,forczlsm=grid%forczlsm,forcwlsm=gri&
&d%forcwlsm,acc_ssoil=grid%acc_ssoil,acc_qinsur=grid%acc_qinsur,acc_qseva=grid%acc_qseva,acc_etrani=grid%acc_etrani,eflxbxy=grid%ef&
&lxbxy,soilenergy=grid%soilenergy,snowenergy=grid%snowenergy,canhsxy=grid%canhsxy,ACC_DWATERXY=grid%ACC_DWATERXY,ACC_PRCPXY=grid%AC&
&C_PRCPXY,ACC_ECANXY=grid%ACC_ECANXY,ACC_ETRANXY=grid%ACC_ETRANXY,ACC_EDIRXY=grid%ACC_EDIRXY,IRFRACT=grid%IRFRACT,SIFRACT=grid%SIFR&
&ACT,MIFRACT=grid%MIFRACT,FIFRACT=grid%FIFRACT,IRNUMSI=grid%IRNUMSI,IRNUMMI=grid%IRNUMMI,IRNUMFI=grid%IRNUMFI,IRWATSI=grid%IRWATSI,&
&IRWATMI=grid%IRWATMI,IRWATFI=grid%IRWATFI,IRELOSS=grid%IRELOSS,IRSIVOL=grid%IRSIVOL,IRMIVOL=grid%IRMIVOL,IRFIVOL=grid%IRFIVOL,IRRS&
&PLH=grid%IRRSPLH,QTDRAIN=grid%QTDRAIN,TD_FRACTION=grid%TD_FRACTION,smcwtdxy=grid%smcwtdxy,rechxy=grid%rechxy,deeprechxy=grid%deepr&
&echxy,fdepthxy=grid%fdepthxy,areaxy=grid%areaxy,rivercondxy=grid%rivercondxy,riverbedxy=grid%riverbedxy,eqzwt=grid%eqzwt,pexpxy=gr&
&id%pexpxy,qrfxy=grid%qrfxy,qspringxy=grid%qspringxy,qslatxy=grid%qslatxy,qrfsxy=grid%qrfsxy,qlatxy=grid%qlatxy,qspringsxy=grid%qsp&
&ringsxy,smoiseq=grid%smoiseq,wtddt=config_flags%wtddt,stepwtd=grid%stepwtd,gecros_state=grid%gecros_state,ua_phys=config_flags%ua_&
...
&d%emiss,HOL=hol,MOL=grid%mol,T2OBS=grid%t2obs,Q2OBS=grid%q2obs,RAINBL=grid%rainbl,SR=grid%sr,RAINSHV=grid%rainshv,GRAUPELNCV=grid%&
&graupelncv,HAILNCV=grid%hailncv,RAINNCV=grid%rainncv,REGIME=grid%regime,T2=grid%t2,THC=grid%thc,QSG=grid%qsg,QVG=grid%qvg,QCG=grid&
&%qcg,SOILT1=grid%soilt1,TSNAV=grid%tsnav,SMFR3D=grid%smfr3d,KEEPFR3DFLAG=grid%keepfr3dflag,DEW=grid%dew,POTEVP=grid%POTEVP,SNOPCX=&
&grid%SNOPCX,SOILTB=grid%SOILTB,rhosnf=grid%rhosnf,precipfr=grid%precipfr,snowfallac=grid%snowfallac,MOSAIC_LU=config_flags%mosaic_&
&lu,MOSAIC_SOIL=config_flags%mosaic_soil,ISURBAN=isurban,MMINLU=TRIM(mminlu),SNOTIME=grid%SNOTIME,RDLAI2D=config_flags%rdlai2d,usem&
&onalb=config_flags%usemonalb,NOAHRES=grid%noahres,TSK_SAVE=grid%tsk_save,ch=grid%ch,fgdp=grid%fgdp,dfgdp=grid%dfgdp,vdfg=grid%vdfg&
&,grav_settling=config_flags%grav_settling,OM_TMP=grid%om_tmp,OM_S=grid%om_s,OM_U=grid%om_u,OM_V=grid%om_v,OM_DEPTH=grid%om_depth,O&
&M_ML=grid%OM_ML,OM_LON=grid%om_lon,OM_LAT=grid%om_lat,okms=1,okme=config_flags%ocean_levels,rdx=grid%rdx,rdy=grid%rdy,msfu=grid%ms&
&fu,msfv=grid%msfv,msft=grid%msft,XTIME=grid%xtime,OM_TINI=grid%om_tini,OM_SINI=grid%om_sini,id=grid%id,omdt=config_flags%omdt,sf_s&
&urface_mosaic=config_flags%sf_surface_mosaic,mosaic_cat=config_flags%mosaic_cat,mosaic_cat_index=grid%mosaic_cat_index,landusef2=g&
&rid%landusef2,TSK_mosaic=grid%TSK_mosaic,QSFC_mosaic=grid%QSFC_mosaic,TSLB_mosaic=grid%TSLB_mosaic,SMOIS_mosaic=grid%SMOIS_mosaic,&
&SH2O_mosaic=grid%SH2O_mosaic,CANWAT_mosaic=grid%CANWAT_mosaic,SNOW_mosaic=grid%SNOW_mosaic,SNOWH_mosaic=grid%SNOWH_mosaic,SNOWC_mo&
&saic=grid%SNOWC_mosaic,ALBEDO_mosaic=grid%ALBEDO_mosaic,ALBBCK_mosaic=grid%ALBBCK_mosaic,EMISS_mosaic=grid%EMISS_mosaic,EMBCK_mosa&
&ic=grid%EMBCK_mosaic,ZNT_mosaic=grid%ZNT_mosaic,Z0_mosaic=grid%Z0_mosaic,HFX_mosaic=grid%HFX_mosaic,QFX_mosaic=grid%QFX_mosaic,LH_&
&mosaic=grid%LH_mosaic,GRDFLX_mosaic=grid%GRDFLX_mosaic,SNOTIME_mosaic=grid%SNOTIME_mosaic,RS_mosaic=grid%RS_mosaic,LAI_mosaic=grid&
&%LAI_mosaic,TR_URB2D_mosaic=grid%TR_URB2D_mosaic,TB_URB2D_mosaic=grid%TB_URB2D_mosaic,TG_URB2D_mosaic=grid%TG_URB2D_mosaic,TC_URB2&
&D_mosaic=grid%TC_URB2D_mosaic,QC_URB2D_mosaic=grid%QC_URB2D_mosaic,UC_URB2D_mosaic=grid%UC_URB2D_mosaic,TRL_URB3D_mosaic=grid%TRL_&
&URB3D_mosaic,TBL_URB3D_mosaic=grid%TBL_URB3D_mosaic,TGL_URB3D_mosaic=grid%TGL_URB3D_mosaic,SH_URB2D_mosaic=grid%SH_URB2D_mosaic,LH&
&_URB2D_mosaic=grid%LH_URB2D_mosaic,G_URB2D_mosaic=grid%G_URB2D_mosaic,RN_URB2D_mosaic=grid%RN_URB2D_mosaic,TS_URB2D_mosaic=grid%TS&
&_URB2D_mosaic,TS_RUL2D_mosaic=grid%TS_RUL2D_mosaic,ZOL=grid%ZOL,SDA_HFX=grid%SDA_HFX,SDA_QFX=grid%SDA_QFX,HFX_BOTH=grid%HFX_BOTH,Q&
&FX_BOTH=grid%QFX_BOTH,QNORM=grid%QNORM,fasdas=config_flags%fasdas,XLAIDYN=grid%XLAIDYN,spp_lsm=config_flags%spp_lsm,pattern_spp_ls&
&m=grid%pattern_spp_lsm,field_sf=grid%field_sf,spp_pbl=config_flags%spp_pbl,pattern_spp_pbl=grid%pattern_spp_pbl,multi_perturb=conf&
&ig_flags%multi_perturb,pert_noah=config_flags%pert_noah,perts_qvapor=grid%pert3d(:,:,:,P_PQVAPOR),perts_th=grid%pert3d(:,:,:,P_PTH&
&),perts_smois=grid%pert3d(:,:,:,P_PSMOIS),perts_tsoil=grid%pert3d(:,:,:,P_PTSOIL),pert_noah_qv=config_flags%pert_noah_qv,pert_noah&
&_t=config_flags%pert_noah_t,pert_noah_smois=config_flags%pert_noah_smois,pert_noah_tslb=config_flags%pert_noah_tslb)
...
&,TKE_PBL=grid%tke_pbl,EL_PBL=grid%el_pbl,WU_TUR=grid%wu_tur,gmt=grid%gmt,xtime=grid%xtime,julday=grid%julday,julyr=grid%julyr,ahe=&
&grid%ahe,distributed_ahe_opt=grid%distributed_ahe_opt,WV_tur=grid%wv_tur,WT_tur=grid%wt_tur,WQ_tur=grid%wq_tur,DISS_PBL=grid%diss_&
&pbl,TPE_PBL=grid%tpe_pbl,TKE_ADV=scalar(ims,kms,jms,P_tke_adv),DISS_ADV=scalar(ims,kms,jms,P_diss_adv),TPE_ADV=scalar(ims,kms,jms,&
&P_tpe_adv),PR_PBL=grid%pr_pbl,EXCH_TKE=grid%exch_tke,RTHRATEN=grid%rthraten,IDS=ids,IDE=ide,JDS=jds,JDE=jde,KDS=kds,KDE=kde,IMS=im&
&s,IME=ime,JMS=jms,JME=jme,KMS=kms,KME=kme,I_START=grid%i_start,I_END=min(grid%i_end,ide-1),J_START=grid%j_start,J_END=min(grid%j_e&
&nd,jde-1),KTS=k_start,KTE=min(k_end,kde-1),NUM_TILES=grid%num_tiles,ZNU=grid%znu,ZNW=grid%znw,MUT=grid%mut,P_TOP=grid%p_top,te_tem&
&f=grid%te_temf,kh_temf=grid%kh_temf,km_temf=grid%km_temf,shf_temf=grid%shf_temf,qf_temf=grid%qf_temf,uw_temf=grid%uw_temf,vw_temf=&
&grid%vw_temf,hd_temf=grid%hd_temf,lcl_temf=grid%lcl_temf,wupd_temf=grid%wupd_temf,mf_temf=grid%mf_temf,thup_temf=grid%thup_temf,qt&
&up_temf=grid%qtup_temf,qlup_temf=grid%qlup_temf,cf3d_temf=grid%cf3d_temf,cfm_temf=grid%cfm_temf,hct_temf=grid%hct_temf,flhc=grid%f&
&lhc,flqc=grid%flqc,exch_temf=exch_temf,QV_CURR=moist(ims,kms,jms,P_QV),F_QV=F_QV,QC_CURR=moist(ims,kms,jms,P_QC),F_QC=F_QC,QR_CURR&
&=moist(ims,kms,jms,P_QR),F_QR=F_QR,QI_CURR=moist(ims,kms,jms,P_QI),F_QI=F_QI,QS_CURR=moist(ims,kms,jms,P_QS),F_QS=F_QS,QG_CURR=moi&
&st(ims,kms,jms,P_QG),F_QG=F_QG,QNIFA_CURR=scalar(ims,kms,jms,P_QNIFA),F_QNIFA=F_QNIFA,QNWFA_CURR=scalar(ims,kms,jms,P_QNWFA),F_QNW&
&FA=F_QNWFA,QNBCA_CURR=scalar(ims,kms,jms,P_QNBCA),F_QNBCA=F_QNBCA,HOL=HOL,MOL=grid%mol,REGIME=grid%REGIME,QKE=grid%qke,Sh3d=grid%s&
&h3d,Sm3d=grid%sm3d,QKE_ADV=scalar(ims,kms,jms,P_qke_adv),bl_mynn_tkeadvect=config_flags%bl_mynn_tkeadvect,tsq=grid%tsq,qsq=grid%qs&
&q,cov=grid%cov,DQKE=grid%dqke,QWT=grid%qWT,QSHEAR=grid%qSHEAR,QBUOY=grid%qBUOY,QDISS=grid%qDISS,tke_budget=config_flags%tke_budget&
&,bl_mynn_cloudpdf=config_flags%bl_mynn_cloudpdf,bl_mynn_mixlength=config_flags%bl_mynn_mixlength,icloud_bl=config_flags%icloud_bl,&
&qc_bl=grid%qc_bl,qi_bl=grid%qi_bl,cldfra_bl=grid%cldfra_bl,bl_mynn_edmf=config_flags%bl_mynn_edmf,bl_mynn_edmf_mom=config_flags%bl&
&_mynn_edmf_mom,bl_mynn_edmf_tke=config_flags%bl_mynn_edmf_tke,bl_mynn_mixscalars=config_flags%bl_mynn_mixscalars,bl_mynn_output=co&
&nfig_flags%bl_mynn_output,bl_mynn_cloudmix=config_flags%bl_mynn_cloudmix,bl_mynn_mixqt=config_flags%bl_mynn_mixqt,bl_mynn_closure=&
&config_flags%bl_mynn_closure,edmf_a=grid%edmf_a,edmf_w=grid%edmf_w,edmf_thl=grid%edmf_thl,edmf_qt=grid%edmf_qt,edmf_ent=grid%edmf_&
&ent,edmf_qc=grid%edmf_qc,sub_thl3D=grid%sub_thl3D,sub_sqv3D=grid%sub_sqv3D,det_thl3D=grid%det_thl3D,det_sqv3D=grid%det_sqv3D,rmol=&
&grid%rmol,ch=grid%ch,qcg=grid%qcg,grav_settling=config_flags%grav_settling,vdfg=grid%vdfg,maxwidth=grid%maxwidth,maxMF=grid%maxmf,&
&ztop_plume=grid%ztop_plume,spp_pbl=config_flags%spp_pbl,pattern_spp_pbl=grid%pattern_spp_pbl,restart=config_flags%restart,cycling=&
&config_flags%cycling,pek=grid%pek_pbl,pep=grid%pep_pbl,PEK_ADV=scalar(ims,kms,jms,P_pek_adv),PEP_ADV=scalar(ims,kms,jms,P_pep_adv)&
&,GWD_OPT=config_flags%gwd_opt,gwd_diags=config_flags%gwd_diags,DTAUX3D=grid%dtaux3d,DTAUY3D=grid%dtauy3d,DUSFCG=grid%dusfcg,DVSFCG&
&=grid%dvsfcg,VAR2D=grid%var2d,OC12D=grid%oc12d,OA1=grid%oa1,OA2=grid%oa2,OA3=grid%oa3,OA4=grid%oa4,OL1=grid%ol1,OL2=grid%ol2,OL3=g&
&rid%ol3,OL4=grid%ol4,SINA=grid%sina,COSA=grid%cosa,dtaux3d_ls=grid%dtaux3d_ls,dtauy3d_ls=grid%dtauy3d_ls,dtaux3d_bl=grid%dtaux3d_b&
&l,dtauy3d_bl=grid%dtauy3d_bl,dtaux3d_ss=grid%dtaux3d_ss,dtauy3d_ss=grid%dtauy3d_ss,dtaux3d_fd=grid%dtaux3d_fd,dtauy3d_fd=grid%dtau&
&y3d_fd,DUSFCG_ls=grid%dusfcg_ls,DVSFCG_ls=grid%dvsfcg_ls,DUSFCG_bl=grid%dusfcg_bl,DVSFCG_bl=grid%dvsfcg_bl,DUSFCG_ss=grid%dusfcg_s&
&s,DVSFCG_ss=grid%dvsfcg_ss,DUSFCG_fd=grid%dusfcg_fd,DVSFCG_fd=grid%dvsfcg_fd,VAR2DLS=grid%var2dls,OC12DLS=grid%oc12dls,OA1LS=grid%&
&oa1ls,OA2LS=grid%oa2ls,OA3LS=grid%oa3ls,OA4LS=grid%oa4ls,OL1LS=grid%ol1ls,OL2LS=grid%ol2ls,OL3LS=grid%ol3ls,OL4LS=grid%ol4ls,VAR2D&
&SS=grid%var2dss,OC12DSS=grid%oc12dss,OA1SS=grid%oa1ss,OA2SS=grid%oa2ss,OA3SS=grid%oa3ss,OA4SS=grid%oa4ss,OL1SS=grid%ol1ss,OL2SS=gr&
&id%ol2ss,OL3SS=grid%ol3ss,OL4SS=grid%ol4ss,MFSHCONV=config_flags%mfshconv,MASSFLUX_EDKF=grid%massflux_EDKF,ENTR_EDKF=grid%entr_EDK&
&F,DETR_EDKF=grid%detr_EDKF,THL_UP=grid%thl_up,THV_UP=grid%thv_up,RT_UP=grid%rt_up,RV_UP=grid%rv_up,RC_UP=grid%rc_up,U_UP=grid%u_up&
&,V_UP=grid%v_up,FRAC_UP=grid%frac_up,RC_MF=grid%RC_MF,phb=grid%phb,XLAT_U=grid%xlat_u,XLONG_U=grid%xlong_u,multi_perturb=config_fl&
&ags%multi_perturb,pert_mynn=config_flags%pert_mynn,perts_qvapor=grid%pert3d(:,:,:,P_PQVAPOR),perts_qcloud=grid%pert3d(:,:,:,P_PQCL&
&OUD),perts_th=grid%pert3d(:,:,:,P_PTH),perts_tke=grid%pert3d(:,:,:,P_PTKE),pert_mynn_qv=config_flags%pert_mynn_qv,pert_mynn_qc=con&
&fig_flags%pert_mynn_qc,pert_mynn_t=config_flags%pert_mynn_t,pert_mynn_qke=config_flags%pert_mynn_qke,Z_AT_W=grid%z_at_w,CLDFRA_OLD&
&_MP=grid%cldfra_old_mp,CLDFRA=grid%cldfra,RTHRATENLW=grid%rthratenlw,TAURESX2D=grid%tauresx2d,TAURESY2D=grid%tauresy2d,TPERT2D=gri&
&d%tpert2d,QPERT2D=grid%qpert2d,WPERT2D=grid%wpert2d,WSEDL3D=grid%wsedl3d,TURBTYPE3D=grid%turbtype3d,SMAW3D=grid%smaw3d,QNC_CURR=sc&
&alar(ims,kms,jms,P_QNC),F_QNC=f_qnc,QNI_CURR=scalar(ims,kms,jms,P_QNI),F_QNI=f_qni,RQNIBLTEN=grid%rqniblten,XLAT_V=grid%xlat_v,XLO&
&NG_V=grid%xlong_v,FNM=grid%fnm,FNP=grid%fnp,IS_CAMMGMP_USED=grid%is_CAMMGMP_used,WSTAR=grid%wstar_ysu,DELTA=grid%delta_ysu,SCALAR=&
&scalar,SCALAR_TEND=scalar_tend,NUM_SCALAR=num_scalar,TRACER=tracer,TRACER_TEND=tracer_tend,NUM_TRACER=num_tracer,SCALAR_PBLMIX=con&
&fig_flags%scalar_pblmix,TRACER_PBLMIX=config_flags%tracer_pblmix,QNORM=grid%QNORM,fasdas=config_flags%fasdas)
