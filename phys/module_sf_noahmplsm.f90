...
"STOP in Noah-MP")
 ...
 CALL PHASECHANGE (parameters,NSNOW   ,NSOIL   ,ISNOW   ,DT      ,FACT    , & 
  ...
  type (noahmp_parameters), intent(in) :: parameters
   ...
   REAL, DIMENSION(1:2), INTENT(INOUT) :: FAB     
   REAL, DIMENSION(1:2), INTENT(INOUT) :: FRE     
   REAL, DIMENSION(1:2), INTENT(INOUT) :: FTD     
   REAL, DIMENSION(1:2), INTENT(INOUT) :: FTI     
   REAL,                 INTENT(INOUT) :: GDIR    
   REAL, DIMENSION(1:2), INTENT(INOUT) :: FREV    
   REAL, DIMENSION(1:2), INTENT(INOUT) :: FREG    
...
"CRITICAL PROBLEM IN MODULE_SF_NOAHMPLSM:VEGEFLUX" )
     ...
     IF(ITER == 1) THEN
        IF (OPT_CRS == 1) then  
         CALL STOMATA (parameters,VEGTYP,MPE   ,PARSUN ,FOLN  ,ILOC  , JLOC , & 
                       TV    ,ESTV  ,EAH    ,SFCTMP,SFCPRS, FVEG, & 
        ...
        IF (OPT_CRS == 2) then  
         CALL  CANRES (parameters,PARSUN,TV    ,BTRAN ,EAH    ,SFCPRS, FVEG, & 
                       RSSUN ,PSNSUN,ILOC  ,JLOC   )          

         CALL  CANRES (parameters,PARSHA,TV    ,BTRAN ,EAH    ,SFCPRS, FVEG, & 
	...
	END IF
        ...
        HCV = parameters%CBIOM*VAIE*CWAT + CANLIQ*CWAT/DENH2O + CANICE*CICE/DENICE    
...
"STOP in Noah-MP")
  ...
  type (noahmp_parameters), intent(in) :: parameters
      ...
      REAL :: T
         ...
         APAR_scale = APAR / max(FVEG,1.0e-6)   
  ...
  SUBROUTINE CANRES (parameters,PAR   ,SFCTMP,RCSOIL ,EAH   ,SFCPRS , FVEG, & 
                     RC    ,PSN   ,ILOC   ,JLOC  )           
       ...
       DO J = ISNOW_OLD+1,0
		 ...
		 ELSE   
		  ...
		  IF(PONDING1 < 0.0) THEN  
		   SICE(1) = SICE(1)+PONDING1/(DZSNSO(1)*1000.0)  
                   PONDING1 = 0.0
		  END IF
                  SNEQV = 0.0
                  SNOWH = 0.0
  ...
  real :: r,x
    ...
    STMSMN  = xsamin/sapm
     ...
     ENDIF
       ...
       FASTCP = FASTCP + (RTTOVR+LFTOVR+STTOVR+DIELF)*DT*0.4  

     FST = 2.0**( (STC-283.16)/10.0 )
     FSW = WROOT / (0.20+WROOT) * 0.23 / (0.23+WROOT)
     RSSOIL = FSW * FST * parameters%MRP* MAX(0.0,FASTCP*1.0E-3)*30.0E-6  

     STABLC = 0.1*RSSOIL   
     FASTCP = FASTCP - (RSSOIL + STABLC)*DT*0.4  
     STBLCP = STBLCP + STABLC*DT*0.4  



     CFLUX  = - CARBFX + (RSLEAF + RSROOT + RSSTEM + RSGRAIN + &
             0.9*RSSOIL + GRLEAF + GRROOT + GRSTEM + GRGRAIN)*0.4  



     NPP    = (NPPL + NPPS+ NPPR +NPPG)*0.4               
     AUTORS = (RSROOT + RSGRAIN + RSLEAF + RSSTEM + &     
               GRLEAF + GRROOT + GRGRAIN + GRSTEM)*0.4    
     HETERS = 0.9*RSSOIL*0.4                              
     NEE    = (AUTORS + HETERS - GPP)*44.0/12.0           
     TOTSC  = FASTCP + STBLCP                             
     TOTLB  = (LFMASS + RTMASS + STMASS + GRAIN)*0.4      
