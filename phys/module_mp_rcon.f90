     ...
     &             0.640961647/)
      ...
      REAL:: dt, pptrain, pptsnow, pptgraul, pptice, pptcloud 
      ...
      i_loop:  do i = i_start, i_end
         ...
         pcp_gr(i,j) = pptgraul
         pcp_ic(i,j) = pptice
         RAINNCV(i,j) = pptrain + pptsnow + pptgraul + pptice + pptcloud				
         RAINNC(i,j) = RAINNC(i,j) + pptrain + pptsnow + pptgraul + pptice + pptcloud 	
         CLOUDNC(i,j) = CLOUDNC(i,j) + pptcloud 	
         ...
         IF (has_reqc.ne.0 .and. has_reqi.ne.0 .and. has_reqs.ne.0) THEN
          ...
          do k = kts, kte
             re_cloud(i,k,j) = MAX(RE_QC_BG, MIN(re_qc1d(k), 75.E-6))	
      ...
      REAL, INTENT(INOUT):: pptrain, pptsnow, pptgraul, pptice, pptcloud 
      REAL, INTENT(IN):: dt
	...
	REAL:: sigma_d
      ...
      do k = kts, kte
		...
		if (qc1d(k) .gt. R1) then
			...
			nc(k) = MAX( 2. , MIN( DBLE(Nt_c_max) , (1./am_r)*rc(k)*EXP(-4.5*ln_sigma(k)*ln_sigma(k))*(1. / ( xDc*EXP(-1.5*ln_sigma(k)*ln_sigma(k)) ))**3.  ) )  
         ...
         if (rr(k).ge. r_r(1)) then
          ...
          if (rg(k).ge. r_g(1)) then
           if (twet(k).lt.T_0) then
            ...
            prr_rcg(k) = -prg_rcg(k)
            pnr_rcg(k) = tnr_racg(idx_g1,idx_g,idx_bg(k),idx_r1,idx_r)  &   
            ...
            pnr_rcg(k) = -1.5*tnr_gacr(idx_g1,idx_g,idx_bg(k),idx_r1,idx_r)  
           endif
          endif
         endif
...
11    CONTINUE
     ...
     &                504,720,990,1320,1716,2184,2730,3360,4080,4896/)
	  ...
	  DOUBLE PRECISION:: ln_sigma,ln_dn,sigma_d 
