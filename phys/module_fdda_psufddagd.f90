...
CONTAINS
   ...
   IF( analysis_interval <= 0 )CALL wrf_error_fatal3("<stdin>",255,&
'In grid FDDA, gfdda_interval_m must be > 0')
   ...
   IF( analysis_interval_sfc <= 0 )CALL wrf_error_fatal3("<stdin>",875,&
'In grid sfc FDDA, sgfdda_interval_m must be > 0')
   ...
   IF( int4 == 1 ) THEN
     coef = (curr_mins2-xtime_old_sfc)/(xtime_new_sfc-xtime_old_sfc)  
   ELSE
     coef = 1.0          
   ENDIF
