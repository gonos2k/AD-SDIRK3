...
MODULE module_mp_nssl_2mom
  ...
  real, private :: rho_qhl=  800., cnohl = 4.0e4 
  ...
  real   , private :: qcwmntim       = 1.0e-5    
      ...
      logical, private :: invertccn = .false. 
  ...
  real   , private :: eiw0 = 0.5          
  real   , private :: esw0 = 0.5          
  ...
  real   , private  :: wetgrthtoffset = -1. 
  real   , private  :: hailcnvtoffset = -2. 
  integer :: ifddenfac = 0  
  ...
  integer :: ibincracr      = 0
  real    :: rainbreakfac   = 1.0e6 
  real    :: draintail      = 10.e-3 
  real    :: drsmall        = 1.e-3 
  real    :: qrbrthresh1    = 0.1e-3 
  real    :: qrbrthresh2    = 1.0e-3 
  ...
  real    :: charging_border = 4000. 
      ...
      integer, private :: imorrgdnglimit = 0      
      ...
      real, save :: dab0lu(ialpstart:nqiacralpha,ialpstart:nqiacralpha,lc:lqmx,lc:lqmx)  
      real, save :: dab1lu(ialpstart:nqiacralpha,ialpstart:nqiacralpha,lc:lqmx,lc:lqmx)  
    ...
    DATA mmgraupvt(:,3) / 0.67819, 0.63789, 0.62197, 0.61240, 0.60572, 0.60066, 0.59663, 0.59330, 0.59048 /
      ...
      real xvhlmn, xvhlmx, xvhlmx0  

      real, parameter :: dhlmn = 0.3e-3
      real, parameter :: dhmn0 = 0.3e-3
      real, private :: dhmn = dhmn0, dhmx = -1.,  dhlmx = -1. 
      ...
      parameter( xvhlmn=0.523599*(dhlmn)**3, xvhlmx0=0.523599*(40.e-3)**3 )  
      ...
      integer, private :: lcn_nu = 0 
      integer, private :: lcn_ac = 0 
      integer, private :: lcn_co = 0 
      integer, private :: lcinp  = 0 
      integer, private :: ac_opt = 0 
      ...
      real,    private :: ac_wthresh = 10.0 
      logical, private :: nuaccoinp = .false.




      logical, private :: arg_para = .true.  
      real,    private :: nu_pmr = 7.5 * 1.e-3 * 1.e-6  
      real,    private :: nu_pgw = 0.53  
      real,    private :: nu_kappa = 0.07  
      real,    private :: ac_pmr = 3.8 * 1.e-2 * 1.e-6  
      real,    private :: ac_pgw = 0.69
      real,    private :: ac_kappa = 0.61  
      real,    private :: co_pmr = 0.51 * 1.e-6  
      real,    private :: co_pgw = 0.77
      real,    private :: co_kappa = 0.61  

      real, parameter :: cn_minlimit = 1.e3  

      logical :: dm15_para = .false.  
 ...
 END FUNCTION fqis
  ...
  character(len=strsize) :: namelist_inputfile
   ...
   logical, optional, intent(in) :: idoniconlytmp
    ...
    namelist_inputfile = 'namelist.input' 
    ...
    ENDIF
      ...
      IF ( hail_on == -1 ) THEN 
        ...
        IF ( ihvol <= -1 .or. ihvol == 2 ) THEN
          ...
          ENDIF
          IF ( ihvol == -2 .or. ihvol == 2 .or. icecrystals_on == 0 ) THEN 
...
SUBROUTINE nssl_2mom_driver(qv, qc, qr, qi, qs, qh, qhl, ccw, crw, cci, csw, chw, chl,  &
      ...
      logical, optional, intent(in) ::  f_zrw, f_zhw, f_zhl, f_vhw, f_vhl 
      ...
      real, dimension(ims:ime, kms:kme, jms:jme),  optional, intent(in) :: elecz 
  ...
  real :: cinchange, t7max,testmax,wmax
   ...
   LOGICAL, optional, intent(in) :: wetscav_on
     ...
     IF ( itimestep == 1 .and. invertccn .and.  flag_ccn ) THEN
      ...
      IF ( invertccn .and.  flag_ccn ) THEN 
        ...
        DO ix = its,ite
          ...
          IF ( lccn > 1 ) THEN
           ...
           ELSEIF ( flag_ccn ) THEN
             ...
             ENDIF
             IF ( i_uf_or_ccn > 0 .and. lccnuf > 1 ) THEN 
    ...
    IF ( loopcnt == 1 ) THEN
     DO il = lnb,na
       IF ( denscale(il) == 1 ) THEN
         DO kz = kts,kte
          DO ix = its,ite
           an(ix,1,kz,il) = an(ix,1,kz,il)*dn1(ix,1,kz) 
          ENDDO
         ENDDO
       ENDIF
     ENDDO 
    ENDIF
     ...
     &            xfall(ix,1,lh)*1000./xdn0(lr) )
         ...
         IF ( present( GRPLNCV ) ) THEN 
           IF ( lhl > 1 .and. .not. present( HAILNC) ) THEN 
   ...
   ENDIF
     ...
     IF (  ( present( ssat3d ) .and. present( nssl_ssat_output )  ) .and. makediag ) THEN
         ...
         ltemq = Min( nqsat, Max(1,ltemq) )
          ...
          IF ( c1 > 0. ) THEN
            ssat3d(ix,kz,jy) = 100.*(an(ix,1,kz,lv)/c1 - 1.0)  
          ENDIF
          
          ENDIF

          IF ( present( ssati ) .and. nssl_ssat_output >= 2 ) THEN
           t9s = (380.0/pn(ix,1,kz))*tabqis(ltemq)  
           ssati(ix,kz,jy) = 100.*(an(ix,1,kz,lv)/t9s - 1.0) 
          ENDIF
...
20         CONTINUE
     ...
     &                    hailmax1d,hailmaxk1,jslab ) 
      ...
      real dtp
      real alpha2d(-nor+1:nx+nor,1,-norz+1:nz+norz,3)  
     ...
     &   timesed1,timesed2,timesed3,zmaxsed,timesetvt) 
      ...
      real, allocatable :: db1(:,:), dtz1(:,:,:),dz2dinv(:,:),db1inv(:,:) 
         ...
         ENDIF
             ...
             IF ( lzr > 1 ) THEN 
               IF ( an(ix,jy,kz,lr) > qxmin_init(lr) .and. an(ix,jy,kz,lzr) < zxmin .and. &
                  ...
                  q = an(ix,jy,kz,lr)
                  nrx = an(ix,jy,kz,lnr)
                  an(ix,jy,kz,lzr) = 36.*g1r*dn(ix,kz)**2*q**2/(pi**2*xdnr**2*nrx) 
               ENDIF
             ENDIF
         ...
         ENDIF
             ...
             IF ( lzh > 1 ) THEN 
               IF ( an(ix,jy,kz,lh) > qxmin_init(lh) .and. an(ix,jy,kz,lzh) < zxmin .and. &
                  ...
                  q = an(ix,jy,kz,lh)
                  nrx = an(ix,jy,kz,lnh)
                  an(ix,jy,kz,lzh) = 36.*g1h*dn(ix,kz)**2*q**2/(pi**2*xdnh**2*nrx) 
               ENDIF
             ENDIF
         ...
         ENDIF
             ...
             IF ( lzhl > 1 ) THEN 
               IF ( an(ix,jy,kz,lhl) > qxmin_init(lhl) .and. an(ix,jy,kz,lzhl) < zxmin .and. &
                  ...
                  an(ix,jy,kz,lzhl) = 36.*g1hl*dn(ix,kz)**2*q**2/(pi**2*xdnhl**2*nrx) 
               ENDIF
             ENDIF
   ...
   implicit none
      ...
      logical, optional :: f_t4, f_t5, f_t6 
   ...
   END SUBROUTINE calc_eff_radius
     ...
     &  infdo,ildo,timesetvt)
      ...
      real, parameter ::  c1h=5.5, c2h=0.7, c3h=4.5, c4h=8.5   
      real, parameter ::  c1hl=3.7, c2hl=0.3, c3hl=9.0, c4hl=6.5, c5hl=1.0, c6hl=6.5 
 ...
 1100 continue
      ...
      IF ( ipconc == 5 .and. imydiagalpha > 0 ) THEN
        DO mgs = 1,ngscnt
          ...
          IF ( qx(mgs,lh) .gt. qxmin(lh) .and. cx(mgs,lh) > cxmin ) THEN
             ...
             xdia(mgs,lh,3) = (xv(mgs,lh)*6.*piinv)**(1./3.) 
     ...
     IF ( lz(lr) > 1 .and. (ildo == 0 .or. ildo == lr ) .and. imurain == 3  ) THEN 
          il = lr
          DO mgs = 1,ngscnt
        ...
        ENDIF
           ...
           IF ( .true. .and. (alpha(mgs,il) <= rnumin .or. alp == rnumin .or. alp == rnumax) ) THEN
            ...
            IF ( rescale_high_alpha .and. alp >= rnumax - 0.01  ) THEN  
              g1 = 36.*(alpha(mgs,lr)+2.0)/((alpha(mgs,lr)+1.0)*pi**2)
      ...
      IF ( ipconc .ge. 6 ) THEN
        ...
        DO mgs = 1,ngscnt
         ...
         ELSEIF ( cx(mgs,il) <= cxmin .and. iresetmoments /= -1 ) THEN 
     ...
     &            ((3.0 + alpha(mgs,il))*(2.0 + alpha(mgs,il))*(1.0 + alpha(mgs,il)))
            ...
            IF ( rescale_high_alpha .and. alp >= alphamax - 0.01  ) THEN  
 ...
 1200 continue
     ...
     &              .and. an(ix,jy,kz,lr) > qhmin) THEN
             ...
             IF ( lzh .gt. 1 ) THEN
              x = (0.224*qh +  0.776*qxw)/an(ix,jy,kz,lh)  
     ...
     &        (hldn*Max(1.0e-9,an(ix,jy,kz,lnhl)))
            ...
            ENDIF 
             ...
             IF ( lzhl .gt. 1 ) THEN 
              x = (0.224*an(ix,jy,kz,lhl) +  0.776*qxw)/an(ix,jy,kz,lhl)  
   ...
   implicit none
      ...
      real, parameter :: mwwater = 0.01801528  
      real, parameter :: rhowater = 997.0  
      real, parameter :: gasconst = 8.3144598   
      real :: sswater  
      real :: sigvl, aact
 ...
 609       dqv  = -(ss1 - 1.)*d1*dtcon
     ...
     &             ((temg(mgs) - cbw)**2))
         ...
         ssmx = ssmxinit
          ...
          IF ( ssf(mgs) > ssmx  .and. ssf(mgs) < 20.0 .and.  &
             ( ccnc(mgs) > 0.05*cwnccn(mgs) .or. ( ac_opt > 0 .and. ccnc(mgs) - cx(mgs,lc) > 0.0 ) ) ) THEN 
     ...
     &      pi0,tabqvs,nqsat,fqsat,cbw,fcqv1,felvcp,ssmx,pk,ngscnt)
      ...
      IF ( dcloud .gt. qxmin(lc) .and. wvel(mgs) > 0.0) THEN
       ...
       CN(mgs) =   CCNE0*cnuctmp**(2./(2.+cck))*wvel(mgs)**cnexp 
  ...
  616 IF ( ssf(mgs) .LE. SUPCB .AND. wvel(mgs) .GT. 0. ) GO TO 631 
     ...
     &     ssfkm1(mgs) .GE. SUPMX)) GO TO 631                      
      ...
      IF ( irenuc >= 0 .and. ac_opt == 0 .and. .not. flag_qndrop ) THEN 
     ...
     &         ( wvel(mgs)*DSSDZ) )  )
      ...
      ELSEIF ( irenuc == 4 ) THEN 
         ...
         ltemq = Min( nqsat, Max(1,ltemq) )
          ...
          ELSE
            ssf(mgs) = 0.0
          ENDIF
       CN(mgs) =   cnuc(mgs)*Min(ssf2kmax, ssf(mgs)**cck) 
      ...
      ELSEIF ( irenuc == 6 ) THEN 
       ...
       IF ( ccna(mgs) < 0.7*cnuc(mgs) ) THEN 
         CN(mgs) =  Min( 0.9*cnuc(mgs), CCNE0*cnuc(mgs)**(2./(2.+cck))*Max(0.0,wvel(mgs))**cnexp )
      ...
      ELSEIF ( irenuc == 7 .or. irenuc == 17 ) THEN 
       ...
       IF ( ccna(mgs) < frac*cnuc(mgs) ) THEN 
         CN(mgs) =  Min( (frac+0.01)*cnuc(mgs), CCNE0*cnuc(mgs)**(2./(2.+cck))*Max(0.0,wvel(mgs))**cnexp )
 ...
 2200 continue
   ...
   END SUBROUTINE NUCOND
     ...
     & )
      ...
      DO il = lc,lhab
       ...
       ELSE
        IF ( il == lc ) THEN
          IF ( ln(il) > 1 ) THEN
           zerocx(il) = ( an(ix,jy,kz,ln(il)) <= 0.0 ) .and. .not. flag_qndrop 
          ENDIF
        ELSE
      ...
      IF ( lzhl .gt. 1 ) THEN
        ...
        IF ( an(ix,jy,kz,lhl) .ge. frac*qxmin(lhl) .and. rescale_low_alpha ) THEN 
     ...
     & )
      ...
      real ac1,bc, c1,d1,e1,f1,p380,tmp,tmp1,tmp2,tmp3,tmp4,tmp5,tmp6,temp3 
      real :: flim, xmass
      real dw,dwr
      ...
      real qhacr(ngs),qhacrmlr(ngs),qhacwmlr(ngs) 
     ...
     &         0.15, 0.37, 0.52, 0.74, 0.82, 0.88, 0.94, 0.98 / 
      ...
      real qeps
      real dyi2,dzi2,bta1,cnit,dragh,dnz00,pii 
 ...
 1100 continue
     ...
     &            ((3.0+alpha(mgs,il))*(2.0+alpha(mgs,il))*rdi) - 1.0
        ...
        IF (  xv(mgs,il) .gt. xvbarmax .or. (il == lr .and. ioldlimiter >= 2 .and. xv(mgs,il) .gt. xvmx(il)/8.)) THEN
          tmp = cx(mgs,il)
          IF( ioldlimiter >= 2 .and. il == lr) THEN 
            x = (6.*rho0(mgs)*qx(mgs,il)/(pi*xdn(mgs,il)*cx(mgs,il)))**(1./3.)
            x1 = Max(0.0e-3, x - 3.0e-3)
            x2 = Max(0.5, x/6.0e-3)
            x3 = x2**3
     ...
     &          ( alpha(mgs,il) <= alphamin .or. alp == alphamin .or. alp == alphamax ) ) THEN
            ...
            ELSEIF ( lrescalelow(il) .and. alp <= alphamin .and. .not. (il == lh .and. icvhl2h > 0 ) .and. &
                     .not. ( il == lr .and. .not. rescale_low_alphar ) ) THEN 
             wtest = .false.
             ...
             chw = g1*dn(igs(mgs),jy,kgs(mgs))**2*(qr)*qr/z*(6./(pi*xdn(mgs,il)))**2 
             cx(mgs,il) = chw
        ...
        DO mgs = 1,ngscnt
          ...
          DO ic = lc,lh-1 
           IF ( il .ne. ic .and.  qx(mgs,ic) .gt. qxmin(ic)) THEN
             xnuc = xnu(ic)
             IF ( ic == lc .and. idiagnosecnu > 0 ) xnuc = alpha(mgs,lc) 
             IF ( il /= lr .and. ic == lr .and. lzr > 1 ) THEN
               IF ( imurain == 3 ) THEN
                 xnuc = alpha(mgs,lr) 
               ELSE
                 xnuc = ( alpha(mgs,lr) - 2. )/3. 
               ENDIF
             ENDIF
                                 
             IF ( .false. ) THEN
             dab0lh(mgs,ic,il) =  delabk(bb(ic), bb(il), xnuc, xnutmp, xmu(ic), xmu(il), 0) 
             dab1lh(mgs,ic,il) =  delabk(bb(ic), bb(il), xnuc, xnutmp, xmu(ic), xmu(il), 1) 
             dab0lh(mgs,il,ic) =  delabk(bb(il), bb(ic), xnutmp, xnuc, xmu(il), xmu(ic), 0) 
             dab1lh(mgs,il,ic) =  delabk(bb(il), bb(ic), xnutmp, xnuc, xmu(il), xmu(ic), 1) 
             ELSE 
               i = Nint( alpha(mgs,il)*dqiacralphainv )
               IF ( ic == lc .or. ic == li .or. ic == ls .or. (ic == lr .and. imurain == 3) ) THEN
                 alp = (3.*alpha(mgs,ic) + 2.)
     ...
     &                 cwmasn,cwmasx,cwradn,cnina,cimn,cimx,   &
     &                 itype1,itype2,temcg,infdo,alpha,axx,bxx,0) 
       ...
       DO mgs = 1,ngscnt
          DO il = lh,lhab 
            ...
            IF ( qx(mgs,il) > qxmin(il) .and. ivshdgs > 0 ) THEN
              ...
              tmpdiam = (shedalp+0.0)*xdia(mgs,il,1) 
     ...
     &                /(temg(mgs)-273.15))
               ...
               ELSEIF ( irimdenopt == 3 .or. irimdenopt == 4) THEN 
           ...
           IF ( qiacr(mgs)*dtp > qxmin(lh) .and. ciacr(mgs) > 1.e-3 ) THEN
             tmpdiam = 1.e6*( 6.*qiacr(mgs)/(1000.*pi*ciacr(mgs) ) )**(1./3.) 
              ...
              IF ( nsplinter .eq. 1001 ) THEN
                ...
                fac = 0.2*Exp(-0.5*((258.-temg(mgs))/10.)**2 ) 
              ENDIF
     ...
     &             ((alpha(mgs,lr) + 3.)*(alpha(mgs,lr) + 2.)*(alpha(mgs,lr) + 1.)) )
      ...
      ENDIF 
        ...
        IF ( icracrthresh > 1 ) THEN
         IF ( imurain == 1 ) THEN
           tmp =  (3.67+alpha(mgs,lr))*xdia(mgs,lr,1) 
         ELSE 
           tmp =  (1.678+alpha(mgs,lr))**(1./3.)*xdia(mgs,lr,1) 
         ENDIF
        ELSE
     ...
     &                  ((alpha(mgs,lr) + 3.)*(alpha(mgs,lr) + 2.)*(alpha(mgs,lr) + 1.))
       ...
       IF ( irainbreak == 1 .or. irainbreak == 10 ) THEN
                crbreak = Max( 0.0,  rainbreakfac* (rho0(mgs)*qx(mgs,lr))**2 ) 
                cracr(mgs) = cracr(mgs) - crbreak 
       ELSEIF ( irainbreak == 2 .or. irainbreak == 20 ) THEN
          
                crbreak = Max( 0.0,  rainbreakfac*(1. - ec0(mgs))*(rho0(mgs)*qx(mgs,lr))**2 ) 
       ...
       ELSEIF ( irainbreak == 12 ) THEN
                crbreak = Max( 0.0, 3.8098 * (rho0(mgs)*qx(mgs,lr))**1.9416 ) 
                cracr(mgs) = cracr(mgs) - crbreak 
             ENDIF
       ENDIF
     ...
     IF ( chmlr(mgs) < 0.0 .and. (ibinhmlr < 1 .or. lzh < 1) ) THEN 
      IF (  ipconc >= 6 .and. lzr .gt. 1 .and. lzh < 1  .and. qx(mgs,lh) > qxmin(lh) ) THEN 
          tmp = qx(mgs,lh)/cx(mgs,lh)
          alp = alpha(mgs,lh)
          g1 = g1x(mgs,lh) 
     ...
     &      .or. (qx(mgs,lr) .le. qxmin(lr) .and. qx(mgs,lc) .le. qxmin(lc)) ) THEN 
        ...
        IF ( qsmlr(mgs) < 0.0 .and. .not. mixedphase ) THEN 
     ...
     &   + fwet2(mgs)*(qhaci(mgs) + qhacs(mgs)) )
       ...
       IF ( incwet == 1 .and. qhwet(mgs) < qhdry(mgs) .and. dhwet(mgs) < dg0thresh ) THEN
               ...
               tmp1 = gaminterp(ratio,alpha(mgs,lh),13,1) 
               tmp2 = gaminterp(ratio,alpha(mgs,lh),12,1) 
               tmp3 = gaminterp(ratio,alpha(mgs,lh), 9,1) 
     ...
     &            tmp3*da1lr(mgs)*xdia(mgs,lr,3)**2 )
        ...
        x = gaminterp(ratio,alpha(mgs,lh),9,1) 
     ...
     &     + fwet2(mgs)*(qhlaci(mgs) + qhlacs(mgs)) )
         ...
         IF ( incwet == 1 .and. qhlwet(mgs) < qhldry(mgs) .and. dhlwet(mgs) < dg0thresh ) THEN
               ...
               tmp1 = gaminterp(ratio,alpha(mgs,lhl),13,2) 
               tmp2 = gaminterp(ratio,alpha(mgs,lhl),12,2) 
               tmp3 = gaminterp(ratio,alpha(mgs,lhl), 9,2) 
     ...
     &            tmp3*da1lr(mgs)*xdia(mgs,lr,3)**2 )
        ...
        x = gaminterp(ratio,alpha(mgs,lhl),9,2) 
          ...
          IF ( wtest  .and. &
           ...
           IF ( qx(mgs,lhl) > qxmin(lhl) .and. dmhlopt > 0 ) THEN
             ...
             IF ( tmp < xmas(mgs,lhl) ) THEN
               ...
               dh0 = (( qxd1*tmp**(1./3.) + qx(mgs,lhl)*xmas(mgs,lhl)**(1./3.))/( qxd1 + qx(mgs,lhl)))**3  
      ...
      IF ( qx(mgs,lh) .gt. qxmin(lh) .and. cx(mgs,lh) .gt. 0.0 ) THEN
        ...
        IF ( lzr > 0 .and. qhshr(mgs) /= 0.0 .and. chshrr(mgs) /= 0.0 ) THEN
         ...
         IF ( temg(mgs) >= tfr ) THEN
           ...
           IF ( (shedalp + alpha(mgs,lh))*xdia(mgs,lh,1) < sheddiam ) THEN 
             z1 = g1*(6.0*rho0(mgs)/(pi*xdn(mgs,lr)))**2*( qhshr(mgs)**2/ chshrr(mgs)  ) 
           ELSE
             z1 = g1shr*(6.0*rho0(mgs)/(pi*xdn(mgs,lr)))**2*( qhshr(mgs)**2/ chshrr(mgs)  ) 
           ENDIF
           zhshrr(mgs) = z1
     ...
     &         ( 2.*tmp * qrfrzf(mgs) - tmp**2 * crfrzf(mgs)  )
      ...
      IF ( qhcns(mgs) > 0.0 .and. chcns(mgs) > 0.0 .and. cx(mgs,ls) > cxmin .and. vhcns(mgs) > 0 ) THEN
        ...
        r = rho0(mgs)*qhcns(mgs)/vhcns(mgs) 
        IF ( imusnow == 3 ) THEN
     ...
     &         ( 2.*tmp * qhcns(mgs) - tmp**2 * chcnsh(mgs)  )
      ...
      IF ( qhcni(mgs) > 0.0 .and. chcnih(mgs) > 0.0 .and. cx(mgs,li) > cxmin .and. vhcni(mgs) > 0 ) THEN
        ...
        r = rho0(mgs)*qhcni(mgs)/vhcni(mgs) 
     ...
     &  - il5(mgs)*zhlcnh(mgs)
      ...
      IF ( lzhl .gt. 1 .or. ( lzr > 1 .and. lnhl > 1 ) ) THEN 
      ...
      IF ( qx(mgs,lhl) .gt. qxmin(lhl) .and. cx(mgs,lhl) .gt. 0.0 ) THEN
        ...
        IF ( lzr > 1 .and. qhlshr(mgs) /= 0.0 .and. chlshrr(mgs) /= 0.0 ) THEN
         IF ( temg(mgs) >= tfr ) THEN
           ...
           IF ( (shedalp + alpha(mgs,lhl))*xdia(mgs,lhl,1) < sheddiam ) THEN 
             z1 = g1*(6.0*rho0(mgs)/(pi*xdn(mgs,lr)))**2*( qhlshr(mgs)**2/ chlshrr(mgs)  ) 
           ELSE
             z1 = g1shr*(6.0*rho0(mgs)/(pi*xdn(mgs,lr)))**2*( qhlshr(mgs)**2/ chlshrr(mgs)  ) 
           ENDIF
           zhlshrr(mgs) = z1
     ...
     &  + pqhli(mgs) + pqhld(mgs)
      ...
      write(iunit,*) il3(mgs)*(qiacrf(mgs)+qracif(mgs))  
      write(iunit,*) Max(0.0, qscev(mgs))   
          ...
          IF ( lz(il) <= 1 .or. ioldlimiter == 1 ) THEN 
           ...
           DO mgs = 1,ngscnt
            IF ( qx(mgs,il) .le. 0.0 ) THEN
              cx(mgs,il) = 0.0
            ELSE 
              IF ( cx(mgs,il) .gt. cxmin .and. qx(mgs,il) > qxmin(il) ) THEN 
     ...
     &              (il == ls .and. imusnow == 3 ) .or. ( il >= lh .and. lh > 0 ) ) THEN
        ...
        ENDIF
          ...
          ENDDO
           ...
           ELSEIF ( il == lh .or. il == lhl .or. il == lf .or. (il == lr .and. imurain == 1 )) THEN 
