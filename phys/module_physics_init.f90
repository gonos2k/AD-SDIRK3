        ...
        DO ls=1,luseas
          ...
          DO LC=1,LUCATS
            IF(found_lu)THEN
              ...
              IF(LC.NE.LI)CALL wrf_error_fatal3("<stdin>",1855,&
...
'arguments not present for calling camzmscheme' )
  ...
  INTEGER :: i,j,itf,jtf
   ...
   mp_select: SELECT CASE(config_flags%mp_physics)
     ...
     CASE (UDMSCHEME)
          CALL udminit(rhoair0,rhowater,rhosnow,cliq,cpv,ccn_conc, allowed_to_read )
     ...
     CASE (NSSL_2MOM)
       ...
       ENDIF
         ...
         IF ( config_flags % nssl_ccn_on > 0 ) THEN
            ccn_conc = config_flags%nssl_cccn/1.225 
         ENDIF
         CALL nssl_2mom_init(nssl_params=nssl_params,ipctmp=nssl_ipconc,mixphase=0, &
           ...
           nssl_icdxhl=config_flags%nssl_icdxhl,                                    &
           ccn_is_ccna=config_flags%nssl_ccn_is_ccna)
