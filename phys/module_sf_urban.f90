...
"slucm_distributed_drag is not compatible with groption" )
   END IF
...
ENDIF
 ...
 if(groption==1) then
   ...
   IF (distributed_aerodynamics_option) THEN
     TS = TA + FLXTH / (ALPHAC / (RHO * CP))  
     QS = QA + FLXHUM / (ALPHAC / (RHO * CP)) 
   ELSE
   TS = TA + FLXTH/CHS    
   QS = QA + FLXHUM/CHS   
   END IF
...
'Error allocating ALBR_TBL in urban_param_init' )
            ...
            if(allocate_status /= 0) call wrf_error_fatal3("<stdin>",2202,&
...
'Error allocating TBLEND_TBL in urban_param_init' )
            ...
            if(allocate_status /= 0) call wrf_error_fatal3("<stdin>",2241,&
...
'Error allocating GR_FRAC_ROOF_TBL in urban_param_init' )
         ...
         if(allocate_status /= 0) call wrf_error_fatal3("<stdin>",2337,&
'Error allocating ROOF_WIDTH in urban_param_init' )
         ...
         if(allocate_status /= 0) call wrf_error_fatal3("<stdin>",2343,&
