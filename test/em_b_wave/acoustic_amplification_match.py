...
print(f"# von-Neumann amplification: SIMPLIFIED model (1-D vertical, CONSTANT metrics/c2a, pure w-phi,")
print(f"#   t'=0, no buoyancy/horizontal/calc_p_rho) — proves coupling STRUCTURE only, NOT the full scheme.")
print(f"#   explicit, no implicit A   : |lambda|_max = {lam_expl:.4f}   (should be >> 1: A is load-bearing)")
print(f"#   semi-implicit (epssm=0.1) : |lambda|_max = {lam_semi:.6f}  (should be <= 1: structure stable, damped)")
...
print(f"# core vertical w-phi coupling STRUCTURE stable (simplified) + A load-bearing : {'PASS' if ok else 'FAIL'}")
print(f"#   NOTE: full-scheme stability (variable c2a(z), buoyancy, 3-D horizontal) NOT proven here —")
print(f"#   requires the Inc 5 assembled-substep-vs-dyn_em [PARITY substep] check.")
