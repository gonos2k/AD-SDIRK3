...
public:
    ...
    // FIX 2025-01-07 Batch41 Round17: Explicit kdim for 2D rdzw on cubic-like grids
    // See wrf_sdirk3_types.h for full documentation. Values:
    //   0 = [nz+1, nx] layout (WRF standard), 1 = [ny, nz+1] layout, -1 = auto-detect
    int rdzw_kdim = -1;

    // FIX 2025-01-08 Batch41 Round30: Explicit kdim for 2D rdz on cubic-like grids
    ...
    float damp_top = 0.0;            // Upper boundary damping
    ...
    // =========================================================================
    // FIX 2025-01-07 Batch41 Round17: IMPORTANT NOTES FOR EXTERNAL CONSUMERS
    // FIX 2025-01-08 Batch41 Round33: Added rdz_kdim documentation
    //
    ...
    //
    // For buoyancy diffusion and acoustic fallback that use rdz:
...
public:
    ...
    int n_species = 0;     // Number of species
    
    // Diffusion options
    float khdif = 0.0;     // Horizontal diffusion coefficient
    float kvdif = 0.0;     // Vertical diffusion coefficient
    int diff_opt = 1;      // 1=2nd order, 2=4th order
};
...
public:
    ...
    // F_fast: Acoustic and compressible terms
    ...
    // F_slow: Advection and rotation terms
    void add_momentum_advection(torch::Tensor& F, const torch::Tensor& u,
                              const torch::Tensor& v, const torch::Tensor& w,
                              const torch::Tensor& rho);
    void add_scalar_advection(torch::Tensor& F, const torch::Tensor& u,
                            const torch::Tensor& v, const torch::Tensor& w,
                            const torch::Tensor& theta);
    ...
    void add_horizontal_diffusion(torch::Tensor& F, const torch::Tensor& u,
                                const torch::Tensor& v, const torch::Tensor& w,
                                const torch::Tensor& theta);
    void add_vertical_diffusion(torch::Tensor& F, const torch::Tensor& u,
                              const torch::Tensor& v, const torch::Tensor& w,
                              const torch::Tensor& theta);
    void add_upper_damping(torch::Tensor& F, const torch::Tensor& u,
                         const torch::Tensor& v, const torch::Tensor& w);
    
    // Physics parameterizations
    void add_physics_tendencies(torch::Tensor& F, const torch::Tensor& U);
    void add_microphysics_tendencies(torch::Tensor& F, const torch::Tensor& U);
    void add_radiation_tendencies(torch::Tensor& F, const torch::Tensor& U);
    void add_pbl_tendencies(torch::Tensor& F, const torch::Tensor& U);
    void add_surface_tendencies(torch::Tensor& F, const torch::Tensor& U);
    void add_cumulus_tendencies(torch::Tensor& F, const torch::Tensor& U);
...
private:
    ...
    torch::Tensor laplacian_2d(const torch::Tensor& field);
    torch::Tensor vertical_laplacian(const torch::Tensor& field);
    torch::Tensor create_damping_profile();
};
