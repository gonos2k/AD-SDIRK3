...
struct TileCaseU03 {
    ...
    std::vector<float> coeff_c1f = std::vector<float>(nw, 1.0f);
    std::vector<float> coeff_c2f = std::vector<float>(nw, 0.0f);
    std::vector<float> coeff_c1h = std::vector<float>(nz, 1.0f);
    std::vector<float> coeff_c2h = std::vector<float>(nz, 0.0f);
    ...
    explicit TileCaseU03(float grid_spacing = 100000.0f, float coriolis = 0.0f,
                         float column_mass = 80000.0f, float metric_abs = static_cast<float>(nz),
                         float map_scale = 1.0f)
        : spacing(grid_spacing), solver(nx, ny, nz, spacing, spacing,
                       {1.0f/spacing}, {1.0f/spacing}, std::vector<float>(nz, metric_abs), 0) {
        metric.assign(nw, metric_abs);
        mass_map.assign(sm, map_scale);
        ...
        std::vector<float> pressure(st), t_initial(st, 0), phi(sw), mass(sm, column_mass);
        ...
        const auto phi_column = wrf::sdirk3::integrate_phb_hydrostatic(
            std::vector<float>(nz,-metric_abs), alpha, std::vector<float>(nz,1),
            std::vector<float>(nz,0), column_mass, 0.0f);
        for (int j=0; j<ny; ++j) {
            for (int k=0; k<nz; ++k)
                for (int i=0; i<nx; ++i)
                    pressure[(j*nz+k)*nx+i] = 100000.0f - (k+0.5f)*column_mass/nz;
            for (int k=0; k<nw; ++k)
                for (int i=0; i<nx; ++i) {
        ...
        setup.c1f_ptr=coeff_c1f.data(); setup.c2f_ptr=coeff_c2f.data();
        setup.c1h_ptr=coeff_c1h.data(); setup.c2h_ptr=coeff_c2h.data();
    ...
    // Re-enter the same accepted zero-copy setup path used by the constructor
    // after changing the private fixture's geometry arrays. This keeps map
    // variation on the production input contract rather than mutating solver
    // cache tensors directly.
    void refresh_setup_maps() {
        wrf::sdirk3::ZeroCopyConfig setup{};
        setup.nx=nx; setup.ny=ny; setup.nz=nz;
        setup.nx_u=nu; setup.ny_v=nv; setup.nz_w=nw;
        setup.ids=setup.its=setup.ims=1; setup.ide=setup.ite=setup.ime=nu;
        ...
        setup.c1f_ptr=coeff_c1f.data(); setup.c2f_ptr=coeff_c2f.data();
        setup.c1h_ptr=coeff_c1h.data(); setup.c2h_ptr=coeff_c2h.data();
        setup.fnm_ptr=setup.fnp_ptr=half.data(); setup.f_ptr=setup.e_ptr=setup.sina_ptr=setup_zero.data();
        setup.cosa_ptr=setup_map.data();
        // Enter the production stage path so metric and coefficient publication
        // precedes the candidate's actual Newton preconditioner refresh.
        solver.advanceZeroCopy(setup, 1, 1.0e-4f);
        TORCH_CHECK(solver.getLastStepOutcomeCode() == 0,
                    "map refresh setup failed: code=", solver.getLastStepOutcomeCode());
    }

    // Republish vertical coefficients through the public zero-copy setup path.
    // This deliberately updates WRFGridInfo, unlike the rejected private tile
    // setter used by the original mathematical fixture.
    void refresh_setup_coefficients(const std::vector<float>& c1f,
                                    const std::vector<float>& c2f,
                                    const std::vector<float>& c1h,
                                    const std::vector<float>& c2h) {
        TORCH_CHECK(c1f.size() >= nw && c2f.size() >= nw &&
                    c1h.size() >= nz && c2h.size() >= nz,
                    "public vertical coefficient publication has insufficient length");
        std::copy(c1f.begin(), c1f.begin() + nw, coeff_c1f.begin());
        std::copy(c2f.begin(), c2f.begin() + nw, coeff_c2f.begin());
        std::copy(c1h.begin(), c1h.begin() + nz, coeff_c1h.begin());
        std::copy(c2h.begin(), c2h.begin() + nz, coeff_c2h.begin());
        refresh_setup_maps();
    }
};
