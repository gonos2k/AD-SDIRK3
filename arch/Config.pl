...
# Check for LibTorch installation for SDIRK-3 solver
 if ( $ENV{LIBTORCH_ROOT} )
 {
   $sw_libtorch_path = $ENV{LIBTORCH_ROOT};
   printf "Found LIBTORCH_ROOT environment variable: %s\n", $sw_libtorch_path;
 }
...
while ( <CONFIGURE_DEFAULTS> )
{
  ...
  if ( $latchon == 1 )
  {
    ...
    if ( index($optarray[$optchoice], "SDIRK-3") > -1 ) {
       $_ =~ s/CONFIGURE_SDIRK3/sdirk3_libtorch/g ;
       # Replace LIBTORCH_PATH placeholder with actual path
       if ( $sw_libtorch_path ne "" ) {
          $_ =~ s/CONFIGURE_LIBTORCH_PATH/$sw_libtorch_path/g ;
       } else {
          # Use default path if LIBTORCH_ROOT not set
          $_ =~ s/CONFIGURE_LIBTORCH_PATH/\/opt\/homebrew\/opt\/pytorch\/libexec\/lib\/python3.13\/site-packages\/torch/g ;
       }
    } else {
       $_ =~ s/CONFIGURE_SDIRK3//g ;
       $_ =~ s/CONFIGURE_LIBTORCH_PATH//g ;
    }
